...
/*
 *   ActionListner: 버튼 / enter / Menu
 *      => actionPerforemd
 *   MouseListener / MouseMotionListener / KeyListener ... 인터페이스
 *   
 *   인터페이스: 처리하는 기능이 여러 개 있다
 *           : 메소드는 선언 => 프로그램에 맞게 구현해서 사용
 *           : 단점 : 선언된 메소드가 많은 경우 => 선택적으로 사용할 수 없다 
 *                                       => 메소드는 {} 넣으면 무조건 구현 완료
 *                                       => 선언된 메소드는 모두 구현해야 한다
 *                                       => 가급적이면 구현해서 전송
 *                                                  ---선택적으로 처리 가능
 *                                                  ==> default
 *                                                  ==> 필요시 해당 메소드만 오버라이딩 가능
 */
public class 퍼즐게임 extends JFrame implements ActionListener,MouseListener{
    ...
    public 퍼즐게임()
    {
    	...
    	JPanel p=new JPanel();//컴퍼넌트 여러 개 묶어 배치
    	
    	JPanel pan=new JPanel();
    	pan.setLayout(new GridLayout(3,3,5,5));//행,열,좌우간격,상하간격
    	for(int i=0;i<9;i++)
    	{
    		btns[i]=new JButton(String.valueOf(i+1));
    		btns[i].setFont(new Font("궁서체",Font.BOLD,35));
    		pan.add(btns[i]);
