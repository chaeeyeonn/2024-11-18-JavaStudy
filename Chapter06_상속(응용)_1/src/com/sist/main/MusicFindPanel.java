...
//행위(마우스, 키보드) => event 발생 => event에 대한 처리
//event 발생하면 => event 정보를 프로그래머로 넘겨준다
//               ----------------------메소드 구현 안 됨 => 추상 / 인터페이스
//웹 => JavaScript 
...
//		p.add(b2);
	 ...
	 *  마우스
	 *  mouseClicked, mousePressed, mouseRelease, mouseEntered, mouseExited
	 *  키보드
	 *  
	 */
	@Override//이벤트 발생에 대한 처리 함수
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		if(e.getSource()==tf)//enter
		{
			//입력 데이터 읽기
			String fd=tf.getText();
			if(fd.trim().length()<1)//입력이 안 되었다면
			{
	...
	public void musicFind(String fd)
	{
		...
		{
			String[] data= {
					String.valueOf(vo.getNo()),vo.getTitle(),vo.getSinger(),vo.getAlbum()};
			model.addRow(data);
			}
		}
