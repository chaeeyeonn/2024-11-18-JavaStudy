...
/*
 *   1. JFrame / JDialog => BoarderLayout
 *   2. JPanel => FlowLayout
 */
public class FoodGenrePanel extends JPanel implements ActionListener,MouseListener{
	...
	JLabel[] imgs=new JLabel[12];// 한 화면에 12개씩 출력
	int curpage=1;
	int totalpage=0;
	JPanel pan=new JPanel();// 이미지 들어갈 곳
	// 데이터베이스 연동
	...
	public FoodGenrePanel(ControlPanel cp)
	{
		...
		pan.setLayout(new GridLayout(3,4,5,5));// 0줄,0개,상하간격,좌우간격
	...
	public void print()
	{
		...
		for(int i=0;i<list.size();i++)
		{
			...
			try
			{
				...
				Image image=ImageChange.getImage(new ImageIcon(url), 450, 300);
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(int i=0;i<genre.length;i++)
		{
			...
			{
				strGenre=genre[i].getText(); // 버튼 문자열 읽기
				// 현재페이지 => 1로 돌리기
				curpage=1;
				init();
				print();
